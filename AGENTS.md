...
- **`IAPConfig.get_coin_reward()` keys** are the internal keys (`instant_coins_1/2/3`), NOT the product IDs. `Boutique.gd` calls `get_coin_reward(sku)` with a product ID, so `get_coin_reward` maps product ID → reward internally (unlike the original Android version which keyed by SKU).
...
- Ads use Unity Ads — the `AdsManager` autoload drives the GDScript `UnityAds` autoload, which wraps the native `GodotUnityAds` iOS plugin singleton (`Engine.get_singleton("GodotUnityAds")`). The iOS plugin lives in `ios/plugins/unity-ads/` (an Objective-C++ bridge built by `build.sh`, run on CI only since it needs macOS/Xcode).
- Product IDs use reverse-domain notation: `com.batikboutique.instant_coins_1/2/3`.
...
- Game ID / placement are configured under Project Settings → `unity_ads/` (`ios/game_id` = `800366520`, `ios/placements/rewarded`). Default rewarded placement is `Rewarded_iOS`.
